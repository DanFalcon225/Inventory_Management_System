...
if($_POST) {	
  ...
  $paymentStatus 				= $_POST['paymentStatus'];
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {		
		//  product table
		$updateProductQuantitySql = "SELECT products.quantity FROM products WHERE products.product_id = ".$_POST['productName'][$x]."";
		$updateProductQuantityData = $connect->query($updateProductQuantitySql);			
			
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			// order item table add product quantity
			$orderItemTableSql = "SELECT orders_items.quantity FROM orders_items WHERE orders_items.order_id = {$orderId}";
			...
			$updateQuantitySql = "UPDATE products SET quantity = $editQuantity WHERE product_id = ".$_POST['productName'][$x]."";
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {			
		$removeOrderSql = "DELETE FROM orders_items WHERE order_id = {$orderId}";
		...
		for($x = 0; $x < count($_POST['productName']); $x++) {			
			...
			while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
				$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
					...
					$updateProductTable = "UPDATE products SET quantity = '".$updateQuantity[$x]."' WHERE product_id = ".$_POST['productName'][$x]."";
					$connect->query($updateProductTable);

					// add into order_item
				$orderItemSql = "INSERT INTO orders_items (order_id, product_id, quantity, rate, total, orders_item_status) 
